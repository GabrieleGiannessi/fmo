...
void printUsage(const char *prog_name) {
  std::cout << "Uso: " << prog_name << " <workers> [pop_size] [generations] [pop_out_path]\n"
            << "   o: " << prog_name << " [OPZIONI]\n"
            << "Esegue l'algoritmo NSGA-II con parallelizzazione OpenMP per il problema FMO.\n\n"
            << "Opzioni:\n"
            << "  -w, --workers <NUM>      Numero di thread/workers OpenMP\n"
            ...
            << "  OMP_NUM_THREADS, FMO_POPULATION_SIZE, FMO_GENERATIONS, FMO_POPULATION_OUT\n";
}
...
int main(int argc, char *argv[]) {
  ...
  int nw = -1;
  if (const char *env_w = std::getenv("OMP_NUM_THREADS")) {
